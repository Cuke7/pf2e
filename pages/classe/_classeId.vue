<template>
  <v-row justify="center" align="center">
    <v-col cols="12" sm="8" md="8">
      ...
      <h1 class="mx-4 my-0" id="dons">Liste des dons</h1>
      <v-card-text>
        <v-row v-for="(don, index) in dons" :key="index" class="px-3">
          <div>
            <v-card-title class="pl-0 primary--text">
              {{ don.translations.fr.name }}</v-card-title
            >
  ...
  computed: {
    dons() {
      let donsClasse = this.$store.state.classfeatures.filter(
        (item) =>
          item.traits.includes(this.item.name.toLowerCase()) && item.level == 1000
      );
      let donsGeneraux = this.$store.state.feats_srd.filter(
        (item) =>
          item.traits.includes(this.item.name.toLowerCase()) && item.level == 18
      );

      return [].concat.apply([], [donsClasse, donsGeneraux]);
    },
    item() {
